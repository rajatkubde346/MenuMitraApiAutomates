    ...
    private void verifyWaiterUpdate(String apiName, String testCaseid, String testType, String description,
        ...
        try
        {
            ...
            if(apiName.equalsIgnoreCase("waiterupdate"))
            {
                ...
                waiterUpdateRequest.setUser_id(String.valueOf(requestBodyJson.getInt("user_id")));
                waiterUpdateRequest.setOutlet_id(String.valueOf(requestBodyJson.getInt("outlet_id")));
                ...
                waiterUpdateRequest.setApp_source(requestBodyJson.getString("app_source"));
                
                // Convert JSONArray to int array for functionality_ids
                org.json.JSONArray jsonArray = requestBodyJson.optJSONArray("functionality_ids");
                int[] functionalityIds = null;
                if (jsonArray != null) {
                    functionalityIds = new int[jsonArray.length()];
                    for (int i = 0; i < jsonArray.length(); i++) {
                        functionalityIds[i] = jsonArray.getInt(i);
                    }
                }
                waiterUpdateRequest.setFunctionality_ids(functionalityIds);
